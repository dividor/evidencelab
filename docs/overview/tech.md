...
![Pipeline Architecture](../images/admin/architecture/pipeline-architecture.png)
...
* **Cross-Lingual features**:
  * **Translation**: Translate search results (titles + snippets) into 10+ languages on request. The translation provider is configurable — Google Translate, a self-hosted LibreTranslate server, or the deployment's own LLM — see [Optional Third-Party Services](../admin/third-party-services.md).
...
![Search Sequence](../images/admin/architecture/search-sequence.png)
...
See [API](../admin/api-keys.md) for full details on key management and usage examples.
