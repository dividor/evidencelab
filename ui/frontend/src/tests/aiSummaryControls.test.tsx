...
describe('AiSummaryControls', () => {
  ...
  test('shows a response variability slider from More Consistent to Creative Insights, defaulting to 0', () => {
    const props = renderControls();
    const slider = screen.getByLabelText('Response variability') as HTMLInputElement;
    expect(slider.value).toBe('0');
    expect(slider.min).toBe('0');
    expect(slider.max).toBe('1');
    expect(screen.getByText('More Consistent')).toBeInTheDocument();
