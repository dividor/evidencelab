...
from utils.tracing import get_trace_recorder  # noqa: E402
...
def get_trace_url(run_id: uuid_mod.UUID) -> Optional[str]:
    """Link to the trace of ``run_id`` from the configured tracing backend.

    ``None`` when tracing is off or the backend cannot build a link.
    """
    try:
        return get_trace_recorder().trace_url(run_id)
    except Exception as exc:
        logger.debug("Could not build trace URL: %s", exc)
        return None
...
async def stream_ai_summary(
    ...
    Yields:
        ...
        ``trace_url`` when LLM tracing is active.
    """
    try:
        # Limit to top N results
        top_results = results[:max_results]
        ...
        # A run id lets the tracing backend correlate this call with a trace
        recorder = get_trace_recorder()
        run_id = recorder.start_run()
        usage_handler = UsageMetadataCallbackHandler()
        config: Dict[str, Any] = {"callbacks": [usage_handler, *recorder.callbacks()]}
        if run_id:
